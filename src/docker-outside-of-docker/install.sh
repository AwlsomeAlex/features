...
if [ "${ID}" = "debian" ] || [ "${ID_LIKE}" = "debian" ]; then
    ...
    for pkg_mgr in dnf microdnf; do
    ...
    if [ -z "${PKG_MGR_CMD}" ]; then
        err "Unable to find a supported package manager (dnf, microdnf)"
        exit 1
    fi
...
# Check if distribution supports Moby
if [ "${USE_MOBY}" = "true" ]; then
    case "${VERSION_CODENAME}" in
        resolute)
            err "The 'moby' option is not supported on Ubuntu '${VERSION_CODENAME}' because Moby packages are not yet available."
            err "To continue, either set feature option '\"moby\": false' or use an earlier Ubuntu release (for example: ubuntu-24.04)."
            exit 1
            ;;
        trixie)
            err "The 'moby' option is not supported on Debian '${VERSION_CODENAME}' because Moby packages are not yet available."
            err "To continue, either set feature option '\"moby\": false' or use an earlier Debian release (for example: bookwork)."
            exit 1
            ;;
    esac

    SUPPORTED_CODENAMES="${DOCKER_MOBY_ARCHIVE_VERSION_CODENAMES}"
    UNSUPPORTED_HINT="either set feature option '\"moby\": false' or choose a compatible OS distribution"
    RHEL_MESSAGE="RHEL-based system (${ID}) detected - Moby packages may require additional configuration"
else
    SUPPORTED_CODENAMES="${DOCKER_LICENSED_ARCHIVE_VERSION_CODENAMES}"
    UNSUPPORTED_HINT="choose a compatible OS distribution"
    RHEL_MESSAGE="RHEL-based system (${ID}) detected - using Docker CE packages"
fi
...
case "${ADJUSTED_ID}" in
    debian)
        ...
        echo "Distro codename '${VERSION_CODENAME}' is supported."
        ;;
    rhel)
        echo "${RHEL_MESSAGE}"
        ;;
...
case ${ADJUSTED_ID} in
    debian)
        check_packages apt-transport-https curl ca-certificates git gnupg2 dirmngr wget
        ;;
    rhel)
        check_packages curl ca-certificates gnupg2 wget tar gawk git shadow-utils policycoreutils procps-ng
        ;;
esac
...
if [ "${USE_MOBY}" = "true" ]; then
    ...
    # Name of proprietary cli package
    ...
    *)
        case ${ADJUSTED_ID} in
            ...
            rhel)
                ...
                set +e # Don't exit if finding version fails - will handle gracefully
                    available_versions=$(${PKG_MGR_CMD} list --available ${cli_package_name} 2>/dev/null | grep -v "Available Packages" | awk '{print $2}' | grep -E "^${docker_version_escaped}" | head -1)
                set -e
    ...
    if [ "${USE_MOBY}" = "true" ]; then
        case "${ADJUSTED_ID}" in
            debian)
                ...
                apt-get -y install --no-install-recommends moby-compose \
                    || err "Package moby-compose (Docker Compose v2) is not available on ${ID} ${VERSION_CODENAME} (${architecture}). Skipping."
                ;;
            rhel)
                err "Moby packages are not available for ${ID}. Please use 'moby': false option."
                exit 1
        ...
        case "${ADJUSTED_ID}" in
            debian)
                ...
                apt-get -y install --no-install-recommends \
                    docker-ce-cli${cli_version_suffix}
                apt-mark hold docker-ce-cli
                # Install Docker CE Compose
                apt-get -y install --no-install-recommends docker-compose-plugin \
                    || echo "(*) Package docker-compose-plugin (Docker Compose v2) not available for OS ${ID} ${VERSION_CODENAME} (${architecture}). Skipping."
                ;;
            rhel)
                ${PKG_MGR_CMD} -y install \
                    docker-ce-cli${cli_version_suffix} \
                ...
                if [ "${DOCKER_DASH_COMPOSE_VERSION}" != "none" ]; then
                    ${PKG_MGR_CMD} -y install docker-compose-plugin \
                        || echo "(*) Package docker-compose-plugin not available for ${ID} ${VERSION_CODENAME} (${architecture}). Skipping."
                fi
                ;;
        esac
    fi
